...
contract RetailOrderEscrow is ERC1155, EIP712, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    bytes32 private constant _TYPEHASH = keccak256("OrderRedeemRequest(address from,string order_id)");
    ...
    function verify(OrderRedeemRequest calldata req, bytes calldata signature) public view returns (bool) {
        address signer = _hashTypedDataV4(keccak256(abi.encode(_TYPEHASH, req.from, keccak256(bytes(req.order_id)))))
            .recover(signature);
